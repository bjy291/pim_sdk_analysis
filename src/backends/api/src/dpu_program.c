...
dpu_load_elf_program_from_elf_info(dpu_elf_file_t *elf_info, struct dpu_program_t *program, mram_size_t mram_size_hint)
{
    ...
    //DPU 프로그램이 실행되기 위해 필요한 주소 및 설정 정보를 포함. (?)
    dpu_elf_get_runtime_info(*elf_info, &runtime_info); //ELF 파일의 각 섹션의 모든 심볼을 검사 -> 실행 시간 정보를 업데이트?

    program->printf_buffer_address = runtime_info.printf_buffer.value; //print 버퍼의 주소 및 크기, 쓰기 포인터 주소, 프로파일링 관련 주소 등 주요 실행 정보 저장
    ...
    if ((symbols = calloc(1, sizeof(*symbols))) == NULL) { //프로그램의 심볼 정보를 저장하기 위한 메모리 공간 할당. //일반 심볼
        result = DPU_ERR_SYSTEM;
        goto free_runtime_info;
    }
    if ((profiling_symbols = calloc(1, sizeof(*profiling_symbols))) == NULL) { //프로파일링 관련 심볼 
        result = DPU_ERR_SYSTEM;
        goto free_symbols;
    }
    ...
    if ((result = dpu_elf_get_sections(*elf_info, &nr_sections, &section_names)) != DPU_OK) { //ELF 파일의 각 섹션 정보 가져오기
        goto free_profiling_symbols;
    }
    elf_fd info = ((elf_fd)*elf_info);

    for (size_t each_section = 0; each_section < info->shnum; ++each_section) {
        char *section_name = section_names[each_section]; // ELF의 각 섹션 순회 
        if (strncmp(".data", section_name, strlen(".data")) == 0) { // 섹션의 심볼을 분석
    ...
    program->symbols = symbols; //심볼 정보를 저장.
    program->profiling_symbols = profiling_symbols; // 프로파일링 심볼 저장

    free(section_names);
    free_runtime_info(&runtime_info); // 섹션 정보를 가져오는데 사용한 메모리 free
...
{
    dpu_error_t result;
    if ((program->program_path = realpath(path, NULL)) == NULL) { //바이너리 파일의 realpath를 구함
        return DPU_ERR_SYSTEM;
    }

    if ((result = dpu_elf_open(program->program_path, elf_info)) != DPU_OK) { //바이너리 파일의 info를 담는듯
        free(program->program_path);
        return result;
    }
    //ELF 메타데이터를 분석 -> DPU 프로그램의 실행에 필요한 정보 추출, ELF의 섹션별로 심볼 정보 로드하고 관리, --심볼 : 프로그램 내의 변수, 함수와 같은 요소를 나타냄. 
    return dpu_load_elf_program_from_elf_info(elf_info, program, mram_size_hint); //문제없을 시 실행 ->
}
