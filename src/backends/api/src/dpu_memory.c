...
dpu_copy_to_mrams(struct dpu_rank_t *rank, struct dpu_transfer_matrix *transfer_matrix)
{
    ...
    verify_mram_access_offset_and_size(offset, size, rank); // MRAM 액세스 유효성 검사

    dpu_lock_rank(rank);

    if (rank->runtime.run_context.nb_dpu_running > 0) { // 랭크에서 실행 중인 DPU 확인. 
        LOG_RANK(WARNING,
            rank,
    ...
    if (rank->description->configuration.mram_access_by_dpu_only) { // mram_access_by_dpu_only 설정에 따라 MRAM에 데이터 복사 방법 결정.
...
copy_to_mrams_using_dpu_program(struct dpu_rank_t *rank, const struct dpu_transfer_matrix *transfer_matrix)
{
    ...
    iram_size_t nr_saved_instructions = 0;
    wram_size_t wram_size_in_words = 0;
    dpuword_t *mram_buffer = NULL; //DPU와 연관된 파라미터 초기화 
