...
{
    dpu_profiling_context_t profiling_context = dpu_get_profiling_context(rank); // profiling_context 반환

    if (profiling_context->thread_profiling_address == 0) // 프로파일링 정보 설정, 
    ...
    if (profiling_context->profiling_symbols == NULL) //프로파일링 심볼 설정
