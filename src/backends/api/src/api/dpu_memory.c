...
dpu_get_symbol(struct dpu_program_t *program, const char *symbol_name, struct dpu_symbol_t *symbol)
{
    ...
    for (uint32_t each_symbol = 0; each_symbol < nr_symbols; ++each_symbol) { // 심볼 순회 
        dpu_elf_symbol_t *elf_symbol = program->symbols->map + each_symbol;
        if (strcmp(symbol_name, elf_symbol->name) == 0) {//심볼name과 일치하는 이름을 가진 심볼 찾기.
            symbol->address = elf_symbol->value; //찾으면 심볼의 주소와 크기를 저장
