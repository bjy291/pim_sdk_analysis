...
dpu_load_rank(struct dpu_rank_t *rank, struct dpu_program_t *program, dpu_elf_file_t elf_info)
{
    ...
    if ((status = dpu_fill_profiling_info(rank,   //프로파일링 info 설정
    ...
    dpu_lock_rank(rank); //rank lock

    struct _dpu_loader_context_t loader_context;
    dpu_loader_fill_rank_context(&loader_context, rank); // 해당 rank에 ELF 파일 로딩하기 위해 rank 정보 입력
