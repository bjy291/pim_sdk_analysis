...
static dpu_error_t
get_section_name(elf_fd info, Elf_Scn *scn, char **name) //ELF 파일 내의 특정 섹션에 대한 name을 동적으로 할당되지 않은 문자열 포인터로 반환
{
    GElf_Shdr shdr;
    if (gelf_getshdr(scn, &shdr) != &shdr) { //섹션의 헤더 가져오기.
        report_error("file corrupted: could not get section header\n");
        return DPU_ERR_ELF_INVALID_FILE;
    }

    if ((*name = elf_strptr(info->elf, info->shstrndx, shdr.sh_name)) == NULL) { //sh_name - 섹션 이름 오프셋. // 섹션 name의 실제 문자열을 가져오기.
...
{
    elf_fd info = (elf_fd)malloc(sizeof(struct _elf_fd)); //elf_info를 위한 메모리 할당
    dpu_error_t err;

    report("dpu_elf_open '%s'\n", path);

    if (elf_version(EV_CURRENT) == EV_NONE) { //라이브러리 버전 확인?
    ...
    clear_elf_fd(info); //info 초기화?

    report("preparing ELF structure\n");
    info->fd = open(path, O_RDONLY, 0); //읽기 전용으로 파일 열기
    if (info->fd == -1) {
        err = DPU_ERR_ELF_NO_SUCH_FILE;
        goto err_invalid_elf;
    }

    info->elf = elf_begin(info->fd, ELF_C_READ, NULL); //파일을 ELF 라이브러리를 사용하여 시작?
    if (info->elf == NULL) { //유효한 형식인지 점검
    ...
    if (elf_rawfile(info->elf, NULL) == NULL) { //elf_rawfile 함수를 사용하여 ELF 파일의 내용을 메모리에 매핑
        err = DPU_ERR_ELF_INVALID_FILE;         //이렇게 하면 나중에 ELF 파일의 다양한 부분에 접근할 때 메모리 할당을 건너뛸 수 있다.
        goto err_invalid_elf;
    }

    err = setup_elf_info(path, info, true); //ELF 파일의 정보를 설정
    if (err != DPU_OK) {
        goto err_invalid_elf;
    }

    *file = (dpu_elf_file_t)info; //ELF 파일의 정보를 저장. elf_info 파람임
    return err;
...
dpu_elf_get_sections(dpu_elf_file_t file, unsigned int *nr_sections, char ***sections)
{
    ...
    *nr_sections = 0; 
    ...
    for (each_section = 0; each_section < info->shnum; each_section++) { // ELF 파일의 섹션 순회?
        Elf_Scn *section_scn = elf_getscn(info->elf, each_section); //Elf_Scn 포인터?
        char *section_name;
        err = get_section_name(info, section_scn, &section_name); //ELF 파일 내의 특정 섹션에 대한 이름을 동적으로 할당되지 않은 문자열 포인터로 반환
        if (err != DPU_OK) {
            if (list != NULL)
                free(list);
            return err;
        }
        list = (char **)realloc(list, (each_section + 1) * sizeof(char *)); // 섹션 name을 저장할 공간 확보
        if (list == NULL) {
            report_error("could not allocate memory to store section names\n");
            return DPU_ERR_SYSTEM;
        }
        list[each_section] = section_name; //섹션 이름 저장
    }
